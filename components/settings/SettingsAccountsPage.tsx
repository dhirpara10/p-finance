  ...
  return (
    <SettingsPanel title="Accounts" onBack={state.goBackSettingsPage}>
      <Field label="Initial Bank balance">
        <input type="number" inputMode="decimal" value={String(state.initialBankBalance)} onChange={(event) => state.setInitialBankBalance(Number(event.target.value))} className="w-full rounded-2xl bg-neutral-800 p-4 outline-none" />
      </Field>
      <Field label="Initial Cash balance">
        <input type="number" inputMode="decimal" value={String(state.initialCashBalance)} onChange={(event) => state.setInitialCashBalance(Number(event.target.value))} className="w-full rounded-2xl bg-neutral-800 p-4 outline-none" />
      </Field>
      <Field label="Monthly reset day">
        <input type="number" inputMode="numeric" pattern="[0-9]*" min={1} max={28} value={String(state.monthlyResetDay)} onChange={(event) => state.setMonthlyResetDay(Math.min(Math.max(Number(event.target.value), 1), 28))} className="w-full rounded-2xl bg-neutral-800 p-4 outline-none" />
      </Field>
      <SelectField
        label="Currency"
