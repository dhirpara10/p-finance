...
function SkeletonCard({ wide = false }: { wide?: boolean }) {
  return (
    <div
      className={`overflow-hidden rounded-3xl border border-white/10 bg-neutral-900/75 p-4 shadow-xl shadow-black/20 ${
        wide ? "col-span-2" : ""
      }`}
    >
      <div className="relative">
        <div className="mb-5 h-3 w-24 rounded-full bg-neutral-800" />
        <div className="h-8 w-32 rounded-full bg-neutral-800" />
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div className="h-14 rounded-2xl bg-neutral-800/80" />
          <div className="h-14 rounded-2xl bg-neutral-800/80" />
        </div>
  ...
  if (error) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-neutral-950 px-4 text-white">
        <div className="w-full max-w-md rounded-3xl border border-red-500/20 bg-neutral-900 p-6 text-center shadow-2xl shadow-red-950/20">
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-neutral-950 px-4 py-8 text-white">
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-5xl flex-col justify-center gap-6">
        <section className="rounded-[2rem] border border-white/10 bg-neutral-900/80 p-5 shadow-2xl shadow-emerald-950/20 backdrop-blur sm:p-6">
